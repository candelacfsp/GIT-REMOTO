	...
	if (inicio != null && fin != null) {
		...
		for (int i = 1; i < usuarios.size(); i++) {
			if (usuarios.get(i).getNombreUsuario()
				...
				String lugar=reporte.CrearPDFFacturaPeriodo(candela, fechaInicio,
						fechaFin, dni, nombre, apellido);
				if (lugar!= null){
				response.sendRedirect("../../FacturasPeriodo.pdf");
				}else{
					candela_sesion.setAttribute("mensaje", "El vendedor no posee facturas en el período seleccionado");
					response.sendRedirect("mostrarFacturaPeriodo.jsp");
				}
			}
		}
	}
