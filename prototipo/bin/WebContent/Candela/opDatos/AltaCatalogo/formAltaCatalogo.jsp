...
<% 
	...
	if(fecha_inicio!=null && fecha_inicio!="" && fecha_fin!=null && fecha_fin!=""){
	
		SimpleDateFormat fechas= new SimpleDateFormat("yyyy-MM-dd");
		
		Date fechaMenor= fechas.parse(fecha_inicio,new ParsePosition(0));
		
		Date fechaMayor= fechas.parse(fecha_fin, new ParsePosition(0));
		...
		if(fechaMayor.before(fechaMenor)){
			...
			<%
				...
				//se crea la fecha actual y se compara con la de inicio
				Date diaActual= fechas.parse("2000-06-04");
				if(fechaMenor.before(diaActual)){
					%>
		...
		//Se setea el catalogo nuevo, como el catalogo que se mantendra en la sesion
	//	candela_sesion.setAttribute("catalogoNuevo", candela.altaCatalogo(fechaMenor, fechaMayor));
...
AC_FL_RunContent( 'codebase','http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=5,0,0,0','width','100','height','22','title','cancelar','src','button1','quality','high','pluginspage','http://www.adobe.com/shockwave/download/download.cgi?P1_Prod_Version=ShockwaveFlash','movie','button1' ); //end AC code
  ...
  <div id="apDiv4">
    <p align="center"><strong><span class="Estilo1">Ingrese las fecha de inicio de vigencia del catálogo y fecha de baja del catálogo.</span></strong></p>
  </div>
</div>
</body>
