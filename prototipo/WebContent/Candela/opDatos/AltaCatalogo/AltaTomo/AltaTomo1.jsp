	...
	if (nroTomo != null && (!nroTomo.equals(""))) {
		...
		if (catalogo.getAnioVigencia() == Constantes.ERROR) {
			...
			if (!response.isCommitted()) {
				...
				int sumatoria = 0;
				for (int j = 0; j < candela.getCatalogoVigente().getTomos().size(); j++) {
				...
				if (colProductos.size() > sumatoria) { //Se verifica que se cuenete con un producto sin asignar a ningun tomo en el sistema.
					...
					try{
						...
						//Si ocurre un error ejecutando altaTomo, entonces se re-establece
						//la referencia de catalogoNuevo de Candela y se redirige a pagina 
						//de error de BD.
